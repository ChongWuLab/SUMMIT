...
# Translate
list.translation           <- readRDS("gencode.v26.hg19.genes.rds")
...
list.supplementary <- readRDS("list.supplementary.rds")
...
while (TRUE) {
    ...
    load(paste0("subset-hapmap3/", filenames[i]))
    ...
    quiet(
        seq.ref <- BEDMatrix(paste0("sequence/1000G.EUR.ALLSNP.QC.CHR", chr), simple_names = TRUE)
    )
    ...
    if (do.adjust) {
        ...
        distance <- as.data.frame(fread(paste0("distance-genetic/subset-omni/chr", chr, ".OMNI.interpolated_genetic_map")))
